...
const Flashcard: React.FC<FlashcardProps> = ({ entry, enableSfx, labels }) => {
  ...
  return (
    <div 
      // REDUCED HEIGHT: h-96 (mobile) -> h-80, md:h-[28rem] -> md:h-96
      className="group w-full max-w-sm md:max-w-md h-80 md:h-96 cursor-pointer perspective-1000 mx-auto transition-all duration-300"
      onClick={handleFlip}
    >
      <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute w-full h-full bg-white rounded-3xl shadow-xl border-b-4 border-pop-purple p-4 flex flex-col items-center justify-center backface-hidden">
            {entry.imageUrl ? (
              // REDUCED IMAGE SIZE: w-60 -> w-48 on desktop to save vertical space
              <img 
                src={`data:image/png;base64,${entry.imageUrl}`} 
                alt={entry.term} 
                className="w-48 md:w-56 h-auto aspect-[2/1] object-cover rounded-2xl mb-4 shadow-sm bg-gray-50 transition-all"
              />
            ) : (
              <div className="w-48 md:w-56 h-auto aspect-[2/1] bg-gray-100 rounded-2xl mb-4 flex items-center justify-center text-gray-300 transition-all">
                <i className="fa-solid fa-image text-4xl"></i>
              </div>
            )}
            {/* Display term in lowercase */}
            <h2 className="text-3xl font-black text-pop-dark text-center mb-1">{entry.term.toLowerCase()}</h2>
            <button 
              onClick={handleAudio}
            ...
            <p className="text-gray-400 text-xs mt-3 uppercase tracking-widest">{labels.tapToFlip}</p>
        </div>
        ...
        <div className="absolute w-full h-full bg-pop-purple rounded-3xl shadow-xl p-6 flex flex-col justify-between backface-hidden rotate-y-180 text-white">
          <div className="overflow-y-auto no-scrollbar">
            ...
            <p className="text-lg font-medium mb-3 leading-snug">{entry.definition}</p>
            
            <h3 className="text-base font-bold opacity-80 mb-1">{labels.example}</h3>
            <p className="text-base italic font-light">"{entry.examples[0]?.target}"</p>
            <p className="text-xs opacity-80 mt-1">{entry.examples[0]?.source}</p>
          </div>
